...
function buildObjectKey({ userId, fileName, moduleName = "misc" }) {
  const cleanedName = sanitizeFileName(fileName);
  const cleanedModule = sanitizeModule(moduleName);
  const stamp = new Date().toISOString().slice(0, 10);
  return `${cleanedModule}/${stamp}/u${userId}-${crypto.randomUUID()}-${cleanedName}`;
}
...
export async function createUploadTarget({ userId, fileName, fileType, moduleName = "misc" }) {
  ...
  const key = buildObjectKey({ userId, fileName, moduleName });
